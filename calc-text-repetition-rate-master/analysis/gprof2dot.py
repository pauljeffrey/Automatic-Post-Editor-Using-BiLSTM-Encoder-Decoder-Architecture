...
class Profile(Object):
        ...
        for call in compat_itervalues(function.calls):
            try:
                ...
                if callee_data.onstack:
                    func_data.lowlink = min(func_data.lowlink,
                                            callee_data.order)
            except KeyError:
    ...
    def _integrate_cycle(self, cycle, outevent, inevent):
        if outevent not in cycle:
            ...
            for callee, call_ratio in compat_iteritems(callees):
                ...
                self._call_ratios_cycle(cycle, callee, ranks, call_ratios,
                                        set())
                ...
                assert abs(call_ratio * total -
                           partial) <= 0.001 * call_ratio * total
                ...
                if call.callee_id != function.id:
                    ...
                    if callee.cycle is cycle:
                        if ranks[callee] > ranks[function]:
                            ...
                            self._call_ratios_cycle(cycle, callee, ranks,
                                                    call_ratios, visited)

    def _integrate_cycle_function(self, cycle, function, partial_ratio,
                                  partials, ranks, call_ratios, outevent,
                                  inevent):
    ...
    def prune(self, node_thres, edge_thres, paths, color_nodes_by_selftime):
            ...
            for call in compat_itervalues(function.calls):
                ...
                if TOTAL_TIME_RATIO in call:
                    ...
                    try:
                        ...
                        call.weight = min(function[TOTAL_TIME_RATIO],
        ...
        for function_id in compat_keys(self.functions):
            function = self.functions[function_id]
            if paths and function.filename and not any(
                    function.filename.startswith(path) for path in paths):
                del self.functions[function_id]
            elif paths and function.module and not any(
                (function.module.find(path) > -1) for path in paths):
                del self.functions[function_id]
...
class XmlToken:
    def __init__(self, type, name_or_data, attrs=None, line=None, column=None):
        assert type in (XML_ELEMENT_START, XML_ELEMENT_END, XML_CHARACTER_DATA,
                        XML_EOF)
        self.type = type
    ...
    def finish_character_data(self):
        if self.character_data:
            if not self.skip_ws or not self.character_data.isspace():
                ...
                token = XmlToken(XML_CHARACTER_DATA, self.character_data, None,
                                 line, column)
                self.tokens.append(token)
            self.character_data = ''
...
class XmlTokenMismatch(Exception):
    ...
    def __str__(self):
        return '%u:%u: %s expected, %s found' % (
            self.found.line, self.found.column, str(
                self.expected), str(self.found))
...
class XmlParser(Parser):
    ...
    def element_start(self, name):
        ...
        if self.token.type != XML_ELEMENT_START:
            raise XmlTokenMismatch(XmlToken(XML_ELEMENT_START, name),
                                   self.token)
...
class GprofParser(Parser):
    ...
    _cg_primary_re = re.compile(
        r'^\[(?P<index>\d+)\]?' + r'\s+(?P<percentage_time>\d+\.\d+)' +
        r'\s+(?P<self>\d+\.\d+)' + r'\s+(?P<descendants>\d+\.\d+)' +
        r'\s+(?:(?P<called>\d+)(?:\+(?P<called_self>\d+))?)?' +
        r'\s+(?P<name>\S.*?)' + r'(?:\s+<cycle\s(?P<cycle>\d+)>)?' +
        r'\s\[(\d+)\]$')
    ...
    _cg_cycle_header_re = re.compile(
        ...
        r'\s+(?P<self>\d+\.\d+)' + r'\s+(?P<descendants>\d+\.\d+)' +
        r'\s+(?:(?P<called>\d+)(?:\+(?P<called_self>\d+))?)?' +
        r'\s+<cycle\s(?P<cycle>\d+)\sas\sa\swhole>' + r'\s\[(\d+)\]$')
...
class AXEParser(Parser):
    ...
    _cg_primary_re = re.compile(r'^\[(?P<index>\d+)\]?' +
    ...
    _cg_parent_re = re.compile(r'^\s+(?P<self>\d+\.\d+)?' +
                               r'\s+(?P<descendants>\d+\.\d+)?' +
                               r'\s+(?P<name>\S.*?)' +
                               r'(?:\s+<cycle\s(?P<cycle>\d+)>)?' +
                               r'(?:\s+\[(?P<index>\d+)\]\s*)?' + r'\s*$')
    ...
    _cg_cycle_member_re = re.compile(r'^\s+(?P<self>\d+\.\d+)?' +
                                     r'\s+(?P<descendants>\d+\.\d+)?' +
                                     r'\s+(?P<name>\S.*?)' +
                                     r'(?:\s+<cycle\s(?P<cycle>\d+)>)?' +
                                     r'\s+\[(?P<index>\d+)\]' + r'\s*$')
    ...
    def parse_function_entry(self, lines):
        ...
        if not mo:
            sys.stderr.write(
                'warning: unrecognized call graph entry (2): %r\n' % line)
            return
        else:
            function = self.translate(mo)
    ...
    def parse_cycle_entry(self, lines):
        ...
        if not mo:
            sys.stderr.write(
                'warning: unrecognized call graph entry (4): %r\n' % line)
            return
        cycle = self.translate(mo)
        ...
        for function in compat_itervalues(profile.functions):
            for call in compat_itervalues(function.calls):
                if call.ratio is not None:
                    ...
                    call[TOTAL_TIME_RATIO] = call.ratio * callee[
                        TOTAL_TIME_RATIO]
...
class CallgrindParser(LineParser):
    ...
    _cost_re = re.compile(r'^' + __subpos_re + r'( +' + __subpos_re + r')*' +
                          r'( +\d+)*' + '$')
...
class OprofileParser(LineParser):
    ...
    def parse_subentry(self):
        ...
        if 'source' in fields and fields[
...
class SleepyParser(Parser):
    ...
    _symbol_re = re.compile(r'^(?P<id>\w+)' + r'\s+"(?P<module>[^"]*)"' +
                            r'\s+"(?P<procname>[^"]*)"' +
                            r'\s+"(?P<sourcefile>[^"]*)"' +
                            r'\s+(?P<sourceline>\d+)$')
...
class PstatsParser:
    ...
    def parse(self):
        ...
        for fn, (cc, nc, tt, ct,
                 callers) in compat_iteritems(self.stats.stats):
    ...
    def graph(self, profile, theme):
        ...
        for _, function in sorted_iteritems(profile.functions):
            ...
            if len(function_name) >= MAX_FUNCTION_NAME:
                ...
                function_name = function_name[:MAX_FUNCTION_NAME -
                                              1] + unichr(0x2026)
...
def main(argv=sys.argv[1:]):
    ...
    optparser.add_option(
        ...
        default=totalMethod,
        help=
        "preferred method of calculating total time: callratios or callstacks (currently affects only perf format) [default: %default]"
    )
    optparser.add_option('-c',
                         ...
                         dest="theme",
                         default="color",
                         help="color map: %s [default: %%default]" %
                         naturalJoin(themeNames))
    optparser.add_option(
        '-s',
        '--strip',
        action="store_true",
    ...
    optparser.add_option(
        ...
        default=False,
        help=
        "color nodes by self time, rather than by total time (sum of self and descendants)"
    )
    optparser.add_option('--colour-nodes-by-selftime',
                         action="store_true",
                         dest="color_nodes_by_selftime",
    ...
    optparser.add_option('--show-samples',
                         action="store_true",
                         dest="show_samples",
                         default=False,
    ...
    optparser.add_option(
        ...
        dest='node_labels',
        help=
        "measurements to on show the node (can be specified multiple times): %s [default: %s]"
    ...
    optparser.add_option(
        ...
        default="",
        help=
        "prune call graph to show only ancestors of specified leaf function")
    optparser.add_option(
        '--depth',
        type="int",
        dest="depth",
    ...
    optparser.add_option(
        ...
        default=1.0,
        help=
        "skew the colorization curve.  Values < 1.0 give more variety to lower percentages.  Values > 1.0 give less variety to lower percentages"
    )
